...
To test out the difference between `View()` and `view_html()`, you will use them on the `mtcars` data frame built into `R`.

Toggle the settings button in the code block so that `show nothing (run code)` so that this block is not part of your homework.
...
In this code block, pass `mtcars` to `view_html()` to view with that function. Again, toggle the settings button in the code block so that `show nothing (run code)` so that this block is not part of your homework.
