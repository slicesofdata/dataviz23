...
GAME_DAT_choice <- GAME_DAT_choice %>%
  select(., c("most pleasurable", "gender", "play frequency", starts_with("categ")))
```
...
```{r}
#unique(GAME_DAT_choice$`most pleasurable`)
#unique(GAME_DAT_choice$gender)
unique(GAME_DAT_choice$play_frequency)

GAME_DAT_choice <- GAME_DAT_choice %>%
  mutate(., 
         most_pleasurable = factor(most_pleasurable),
         gender = factor(gender),
         play_frequency = factor(play_frequency, 
...
- pass `.fns = ~na_if(.x, val_to_replace)` to the function that will allow you to perform the conversion on many variables (hint: we've used it often). 

```{r}
names(GAME_DAT_choice)

GAME_DAT_choice <- GAME_DAT_choice %>%
  mutate(., across(starts_with("categ"), ~na_if(.x, 99)))
```
...
```{r}
write.csv(GAME_DAT_choice, 
          paste(data_dir, "GAME_DAT_HW04.csv", sep = "/"), row.names = F)
```
...
```{r}
view(GAME_DAT_choice)
names(GAME_DAT_choice)

GAME_DAT_choice %>%
  summarise(., across(.,
                      .cols = c("educational", "economic", 
                                "fantasy", "humor", "political", 
                                "word_game", "math", "party_game") ,
                      .fns  = ~mean(na.omit(.x))
                      ))
```
...
GAME_DAT_choice %>%
  #filter(., complete.cases(.)) %>%
  group_by(., gender, most_pleasurable) %>%
  summarise(., across(.,
                      .cols = c("educational", "economic", 
                                "fantasy", "humor", "political", 
                                "word_game", "math", "party_game") ,
