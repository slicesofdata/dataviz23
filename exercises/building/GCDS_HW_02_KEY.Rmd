...
Use the function to *combine* 10 numeric elements and assign them to a vector object named v.
...
2) extract the 4th through 5th elements
...
# Pasting/Concatenating character objects 
...
Second, now that you know what the path looks like, create a new object named *game_dat_path* that uses `paste()` to concatenate the path with the file name for the data file `"GCDS_Games_Survey_choice.csv"`.

```{r}
game_dat_path <- paste(data_dir, "GCDS_Games_Survey_choice.csv", sep = "/")
```
...
Third, find out whether you have this file saved on your computer by passing this object into a function named `file.exists()`. This function will return a logical TRUE or FALSE.

```{r}
file.exists(game_dat_path)
```
...
```{r}
GAME_DAT <- readr::read_csv(game_dat_path)
```
...
# Using vector objects to subset data frames
