...
geom_pointrange()
descriptives(DATA, groupby = c(), var = )
DATA |>
  group_by(Athlete, Year) |>
  slice_max(Score) |>
  ungroup() |>
  ggplot(mapping = aes(Athlete, Score)) +
  geom_point()
  

  geom_pointrange_(Athlete, Score, median) 
...
for (name in DATA$Athlete) {
  c = ifelse()
  append()
}
  
DATA |> 
  #group_by(Athlete, Year) |> #slice_max(Score) |> 
  #ungroup() |> 
  #filter(Athlete == "Palunas, Vaiva")
  descriptives(groupby = c(Athlete), var = Score) |>
  filter(!is.na(min) & !is.na(mean) & !is.na(max)) |>
  #?cor
  #cor(x = skew, y = mean, method = "pearson") #pull(min)
  ggplot(aes(x = Athlete)) +
  geom_pointrange(aes(forcats::fct_reorder(.f = Athlete, 
                                    .x = mean, 
                                    .fun = function(x) {mean(x)}), 
                                    max)) + 
  coord_flip()
  
    geom_linerange(#y = mean,
                  ymin = min, 
                  ymax = max
                  )
