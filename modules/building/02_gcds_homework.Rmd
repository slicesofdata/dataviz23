...
this_rmd_path <- rstudioapi::getActiveDocumentContext()
...
For this homework, you will save the `R Markdown` homework document and save it to "GCDS/r" on your computer. Rename is **"GCDS_HW_02_yourname"**. When you open it, add your name to the author part of the YAML code. The homework will need to be submitted as a `knit` HTML file.
...
```{r}
game_dat <- paste(data_dir, "GCDS_Games_Survey_choice.csv", sep = "/")
```
...
```{r}
GAME_DAT <- readr::read_csv(game_dat)
```
...
# Getting data frame column names (xx points)
...
GAME_DAT %>%      # take this object
  select(., 1:5)  # pass the object from above into select() and return a                   # new object containing only what is selected 
```

In some cases, however, you do not need to use the period as a stand-in for the data frame. For example:
