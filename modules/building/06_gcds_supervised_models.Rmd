...
## *Model: Simple/Bivariate Linear Regression*
...
compare_performance(lm_stroop, 
                    aov_trialtype,
                    lm_incongruent, aov_incongruent)

```

# *Adding Predicted and Residuals to the Data Frame*

Using the `modelr` library, you can easily add model residuals and predicted values to the data frame producing the model.

In some instances, when cases are dropped from the model (e.g., `NA` values), the starting data frame may not be the same as the model data frame. Grabbing the modeled data frame, however, is easy because it can be extracted from the model itself using the the `$` operator. If your model object is `lm_stroop`, tehn `lm_stroop$model` will return the model. Let's make an object holding the modelled data and name it `stroop_mod_dat`. 

Using `add_predictions()` and `add_residuals()`, we can add them to `stroop_mod_dat`. We will also add an `id` variable based on the rows in the data frame. 

Let's step through how we could do this. There are a few ways we can do it, so this example represents one way. 

- *First*, remember that `dim()` will return a vector contaning the length of the dimensions of a data frame. There are two elements, the first for the length of rows and the second for the columns. We want the rows, so we can extract the first element `dim(stroop_mod_dat)[1]`. 

-  *Second*, because each respondent has 2 rows of data, we want to make sure to assign the same `id` value for consecutive rows of congruent and incongruent data. Thus, we need to cut the row length in half by dividing by 2 or multiplying by .5; `dim(stroop_mod_dat)[1]/2`.

- *Third*, we want to make a sequence of numbers from 1 to half the full length or the reduced length referenced in the point above. Remember that `seq()` will allows us to make a sequence of values from one number to another number, which in our case is have the row length. Make sure to treat the length as a single value by wrapping in in parentheses or otherwise the operations will not be performed in the order to want them to be performed. For example, compare `seq(1 : (dim(stroop_mod_dat)[1]/2))` with `seq(1 : dim(stroop_mod_dat)[1]/2)`. We want `seq(1 : (dim(stroop_mod_dat)[1]/2) )`.

- *Fourth*, remember that `rep()` will replicate a vector some numer of times. Because we have congruent and incongruent trials, we want two replications, so `rep( seq(1 : (dim(stroop_mod_dat)[1]/2)), times = 2)`.

- *Fifth*, using `sort()` we can sort the vector in numerical order so that every two rows get the same `id`. We can now add 
`sort( rep( seq(1 : (dim(stroop_mod_dat)[1]/2)), times = 2))` to mutate a new variable.


```{r}
stroop_mod_dat <- lm_stroop$model

stroop_mod_dat <- stroop_mod_dat %>%
  mutate(., id = sort( rep( seq(1 : (dim(stroop_mod_dat)[1]/2)), times = 2)))

```


We can also make a function to do this, though it would need to be flexible to adjust for the levels of a specified variable.

```{r}
add_res_pred_to_model = function(model = NULL, 
                                 id_by = NULL, 
                                 #data = NULL, 
                                 drop_id = F) {
  # best is passes a model
  if (is.null(model) & is.null(data)) {
    message("'model' and 'data' are NULL. One needs to be specified.")
  } else {
  # if the model is specified  
  if (!is.null(model)) { 
    m = model$model 
    m = m %>% 
      modelr::add_residuals(model) %>%  # add resid 
      modelr::add_predictions(model)    # add predicted
    }
  
  #if (!is.null(data)) m = data
  ...
  m = m %>% 
    ...
    arrange(., id) # resort
  if (drop_id) m = select(m, -id)
  return(m)
  }
}


stroop_mod_dat <- add_res_pred_to_model(model = lm_stroop, id_by = "trialtype") 
```
...
STROOP
STROOP_agg
stroop_mod_dat$id2 = STROOP_agg$id
head(stroop_mod_dat)

stroop_mod_error <- lm(error ~ trialtype, data = STROOP_agg)
stroop_mod_error_dat <- add_res_pred_to_model(stroop_mod_error, "trialtype")
stroop_mod_error_dat$id2 <- STROOP_agg$id

stroop_mod_error_dat %>% 
  ggplot(.) +
  geom_density(aes(x = error, color = trialtype)) +
  see::theme_modern()

stroop_mod_error_dat %>%
  ggplot(., aes(group = id)) +
    geom_jitter(aes(trialtype, error), width = 0) +
    see::theme_modern()

error_p <- stroop_mod_error_dat %>% 
  ggplot(., aes(group = id)) +
  geom_point(aes(trialtype, error)) +
  geom_point(aes(trialtype, pred), size = 2, shape = 1, color = "green") + #, width = .2, alpha = .5, color = "red") +
  geom_text(aes(x = trialtype, y = error, label = id2), nudge_x = c(-.15, .13), color = "black") +
  geom_line(aes(trialtype, error), color = "red", alpha = .4) +
  geom_line(aes(trialtype, pred), color = "green", alpha = 1) +
  geom_line(aes(1.5, pred), color = "black", alpha = 1) +
  #stat_summary(aes(trialtype, pred), geom = "pointrange") +
  #geom_text(aes(x = 1.5, y = pred), label = mean(pred)) color = "black") +
  see::theme_modern()
  
  
rt_p <- ggplot(stroop_mod_dat, aes(group = id)) +
  geom_jitter(aes(trialtype, rt), width = 0) +
  geom_point(aes(trialtype, pred), size = 2, shape = 1, color = "green") + #, width = .2, alpha = .5, color = "red") +
  geom_text(aes(x = trialtype, y = rt, label = id2), nudge_x = c(-.15, .13), color = "black") +
  geom_line(aes(trialtype, rt), color = "red", alpha = .4) +
  geom_line(aes(trialtype, pred), color = "green", alpha = 1) +
  geom_line(aes(1.5, pred), color = "black", alpha = 1) +
  #stat_summary(aes(trialtype, pred), geom = "pointrange") +
  #geom_text(aes(x = 1.5, y = pred), label = mean(pred)) color = "black") +
...
?geom_text #geom_text(aes(x = trialtype, y = pred, label = id), color = "red")


  annotate("text", x = 1.5, y = 500, label = "my plot", color = "black")

  geom_text(mapping = aes(id, pred))
?geom_text

#%>%
  group_by(., trialtype) %>% 
  summarize(., 
            resid_mn = mean(resid*resid),
            resid_sd = sd(resid)
            )
  
plot_resid <- function(data, x = "trialtype", color = "red", width = .3) {
  ggplot(data, aes(x = data[[x]])) +
    geom_ref_line(h = 0) +
    geom_jitter(aes(y = resid), color = color, width = width) +
    xlab(x)
}
plot_resid(stroop_mod_dat, "trialtype")
