...
Using the `gap_stat` method will involve bootstrapping procedures so this will run for quite some time. Do not execute unless you need to get a coffee. This methods suggests that 4 clusters could be best. Essentially, the gap statistic represents a comparison of the total intra-cluster variation for different values of *k* with their expected values for a distribution without clustering. More on the gap statistic can be found in Tibsharani, Walther, & Hastie (2001). https://hastie.su.domains/Papers/gap.pdf
...
By default, we will accept the *Hartigan-Wong algorithm*. You can see in the R documentation that H-W computes the center of gravity of data as well as the distances of data from/to this center point. It then sorts the data set in any order. The *k* cluster prototypes are determined by using the formula (1 + (i - 1)n/k), where *i* = cluster index and *n* = number of observations (or rows). This algorithm leads to increase in the computational cost due to complexity of sorting, O(n log(n)); see also Celebi et al. (2013).

See: Hartigan, J.A. & Wong, W.A., (1979). Algorithm AS 136: A K-means clustering algorithm, J of the Royal Statistical Society, C 28 (1): 100-108.
...
WINE_z$cluster6 <- WINE_km6$cluster
```
...
WINE_km6 %>%
  factoextra::fviz_cluster(
  object = ., 
  data = dplyr::select(WINE_z, -c(color, cluster3, 
                                  cluster4, cluster6)),
  ellipse.type = "convex",
  palette = "jco",
  ggtheme = theme_minimal()
...
factoextra::fviz_nbclust(IRIS[2:5], kmeans, method = "gap_stat")
```
...
Create 2 clusters or 3. Because there are 3 species, let's just use 3 for now and see if the measurement variables actually group in such a way that members of each species group in the same cluster.
...
```{r}
p1 <- ggplot(IRIS, aes(Petal.Length_z, Petal.Width_z, 
                 color = Cluster)) + 
  geom_point(position = "jitter", alpha = .5) + 
  ggtitle("Iris cluster Plot") + see::theme_modern()

p2 <- ggplot(IRIS, aes(Petal.Length_z, Petal.Width_z, 
                 color = Species)) + 
  geom_point(position = "jitter", alpha = .5) + 
  ggtitle("Iris Species Plot") + see::theme_modern()


gridExtra::grid.arrange(p1, p2)
```

## *Plotting a Matrix of Clustering Relative to Species*

Using `table()`, create a table of Species (columns) and Cluster Number (rows).  

```{r}
table(IRIS$Cluster, IRIS$Species)
```

Although the clusters are not classifying the iris rows, most of the species are falling into relative categories. If the clustering is for example creating clusters based on the species, we can think about this classification in terms of accuracy and evaluate a confusion matrix (note: this is not the same a true classificaiton methods).
 

```{r}
IRIS.table <- table(IRIS$Cluster, IRIS$Species)

rownames(IRIS.table) <- c("setosa", "versicolor", "virginica")

IRIS_cm <- caret::confusionMatrix(IRIS.table)
