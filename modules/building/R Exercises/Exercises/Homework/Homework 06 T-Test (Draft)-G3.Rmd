...
- bwplot() for examining data visually; lattice library            bwplot(x ~ A)
...
## 2. Loading libraries ##
Use the library() function to load the following libraries: lattice, nortest
...
You do a  *t*-test for dependent means in exactly the same way as a one-sample *t*-test, except that: 
...
To show this we are going to read in pre-test and post-test scores from a group of students who ranked happiness scores before and finals week and assign them to a data-frame object named HAPPY.
...
Boxplots are a nice way to visually examine data and are much more informative than bar graphs.

```{r}
boxplot(HAPPY$Pre, HAPPY$Post,
        #xlab = "Before and After",
#        names = c("Before", "After"),
        col = c("red", "red"),
        ylab = "Happiness",
        main = "Happiness Before and After\n Final Exams") # \n stand for new line
```


As you can see we have student ids, pre-test scores, and post-test scores. Now we need to create the difference scores so we can analyze the data to examine if the difference is close to or far away from 0. 
```{r}
HAPPY$Diff <- HAPPY$Pre - HAPPY$Post
View(HAPPY)
str(HAPPY)

mean.diff <- mean(HAPPY$Diff)
mean.diff
```
...
The general formula is:  t.test(X1, X2, paired = TRUE)
...
By substitution and making sure to italicize xxxx and p, we have:
  ...
  Students' happiness ratings were statistically higher following final exams than they were before those exams, *V*(19) = 3.38, *p* < .05.*xxx*(xx) = x.xx, *p* < .05
...
One measure of effect size is the Pearson's r correlation coefficient, *r*, which can range from -1.0 to +1.0. Cohen suggested that we could classify the size of the *r* such that Small effects were about *r* = 0.10, Medium, *r* = 0.30, and Large *r* = 0.50. Related, *r*-squared represented an estimate of the amount of the variance within an experiment that is "accounted for" by the statstical model (e.g., linear regression). This can range from 0 to 1.0. Using *r*-squared, Small effects were about *r* = 0.xx, Medium, *r* = 0.xx, and Large *r* = 0.xx. 

Effect Size using r-squared:
?- .01, Small
?- .09, Medium/Moderate
?- .25, Large
...
4.3.4. Using *r*-squared as a measure of effect size for paired samples is a little more challenging. Although the relationship between *r* and *t* will not be described here, you can estimate *r*-squared using the information returned from the t.test() function:
...
5.1. One question asked on the survey was the percentage of a bill left as a tip for a server at a restaurant. To help you, the following code will provide you the class data needed to answer the question. 
...
**QUESTION:** For a population of men and women, a very large data set reports that the average tipping percent is 16.08026%. Use the appropriate test function to determine whether the people responding to the class survey they were generous or cheapskates in their tipping behavior relative to those in the population. 

*ANSWER:*
```{r}
tipping.t <- t.test(tippercent, mu = 16.08026, data = SURVEY)
tipping.t
```

**QUESTION:**
Test whether a normal distribution is a reasonable assumption for those data. Test with alpha = .05. 

*ANSWER:*
```{r}
shapiro.test(SURVEY$tipping)
```

5.2. Use the SURVEY data frame to determine if there is a 

On average, how many hours a week do you exercise?/7
...
**QUESTION:** Name the appropriate test used to determine is a normal distribution is a reasonable assumption for the data. 

*ANSWER:*


**QUESTION:** Use the appropriate function to test this assumption. 
...
**QUESTION:** Use the appropriate function to analyze the data to answer the questions. 
...
**QUESTION:** How much weight was lost on average?

*ANSWER:*


**QUESTION:** Did the program influenced weights more than expected due to chance. 

*ANSWER:*


**QUESTION:** Is the difference significantly different from 0 when alpha = .05? Explain how you arrived at your decision.

*ANSWER:*


**QUESTION:** What are the degrees of freedome for the test?
