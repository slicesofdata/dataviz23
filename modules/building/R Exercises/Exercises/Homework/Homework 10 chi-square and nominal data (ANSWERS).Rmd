...
*ANSWER:* One IV, more than 2 levels. For 2 levels, use the binomial or McNemar tests. 
...
*ANSWER:* There are two IVs, each with two levels, each are categorical in nature and not continuous; Both IVs/factors are elso between subjects. Thus, the test could be a factorial ANOVA, Friedman test, or chi-squared test for independence. The DV will help with the decision. The question is about a "difference in the number of friends" across the groups. The DV is the number of friends across the 25 people in each teatment condition, and the number of anything is ratio because 0 would indicate no friends in this example. Thus, a 2 x 2 between-subjects factorial ANOVA is the appropriate test.
...
*ANSWER:* Following from above, the test might be a factorial ANOVA, Friedman test, or chi-squared test for independence. In this case, the DV is based on a frequency (out of 25) who have more than 5 close friends. The test would be a chi-squared test for independence in order to dermine whether there is a relationship between the categorical IVs.


4. **QUESTION:** What is the difference between the $\chi$^2^ goodness-of-fit test and the $\chi$^2^ test for independence?

*ANSWER:* The goodness-of-fit test involves only one IV/factor and the test for independence involves more than one IV/factor.


5. **QUESTION:** What assumption is there for $\chi$^2^ tests and expected frequencies? 

*ANSWER:* Expected frequencies should be 5 or more.
...
1. **QUESTION:** Use the SURVEY data frame to determine if there is a relationship between beliefs about global issues and gender. Make sure to examine the variables of interest so that you can create appropriate labels for your variables. 
...
```{r}
# get file/data frame if already loaded
#setwd("c:/users/gcook/desktop/Psyc109")
#SURVEY <- read.csv("SurveyNames.csv")

# get relevant variables for the 
Global <- SURVEY$GlobalIssue
Gender <- SURVEY$Gender

# assign to a small data frame
GG <- data.frame(Global, Gender)
...
# create Gender factor and add to the data frame, specify levels and labels
GG$Gender.f <- factor(GG$Gender, levels = c(0, 1), labels = c("Men", "Women"))
...
GG$Global.f <- factor(GG$Global, 
                    levels = c(1, 2, 3, 4, 5, 6, 7), 
                    labels = c("Climate Change", "Terrorism", "Poverty", "Education",
                               "Food Scarcity", "Financial Collapse", "Other"))

# recode/assign any values to NA now before cleaning up the data frame

# take another look.
head(GG)

# make the data set contain only the vector variables relevant for the chi-square test; dropping out any NA on any variables.
GG <- subset(GG, complete.cases(GG), select = c(Gender.f, Global.f))

# make a contingency table (should only have your variables of interest)
t <- table(GG)
GG.chi <- chisq.test(t)   # or GG.chi <- chisq.test(table(GG))
GG.chi

# however, examine the observed frequencies to see what's actually going on with the data 
GG.chi$observed

# and examine the expected values to see if less than 5 
GG.chi$expected

# do Fisher exact because fe < 5 
GG.fish <- fisher.test(t)
GG.fish  # this shows the same as the chi-squared test

# for an effect size, pass the table object not the Chi-squared object to the effect-size function
# library(vcd)
assocstats(t)
```


*INTERPRETATION:* The df make sense, (r - 1)(c - 1), (1)(6) = 6 for this analysis. If using an alpha = .05 and *p*-value, there does not appear to be a relationship, or dependency, between gender and beliefs about current global issues. In other words, the two groups of gender measured here seem to have similar beliefs regarding global issues. However, this does not mean that using different groups of gender identity or different global issues would not reveal a dependence.  

Examining the effect size rather than the *p*-value, the Cramer's V is .31, which could be interpreted as a weak relationship.


And if you wanted a rough barplot:

```{r}
barplot(t) # it's stacked by default (beside argument = TRUE) 

# If you want separate bars
barplot(t, main="Gender and Beliefs about Global Isses",
        xlab="Global Issues", ylab = "Observed Frequencies",
        col = c("Blue","Green"),
        legend = rownames(t), beside=FALSE)
...
# the labels and such can be customized to fit appropriately too.
```
