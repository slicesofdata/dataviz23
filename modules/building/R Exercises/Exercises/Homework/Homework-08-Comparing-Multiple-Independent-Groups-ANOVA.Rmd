...
*OPTIONAL:*
7.2. Because ANOVA is a special case of regression for which the predictor is categorical rather than continuous, we can still create a linear model using `lm()`. And, because you know the `R` output for a linear model provides you with r-squared measures, you can also verify the that ANOVA and regession are the same. 

Build a linear model and examine the estimates:
```{r}
JOBS.lm <- with(JOBS, lm(Startup ~ Shop.fact))
summary(JOBS.lm)
```
Notice, that the (multiple) R-squared value in the output is the sames as that calculated above.
