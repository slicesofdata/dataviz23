...
```{r}
library(lattice)
densityplot(~ SURVEY$Exercise | SURVEY$FavTime.fact, layout = c(1,4))
```
...
*CODED ANSWER:*
```{r}
EXERCISETOD.aov <- aov(SURVEY$Exercise ~ SURVEY$FavTime.fact)
```
...
```{r}
# Tukey's HSD test
TukeyHSD(EXERCISETOD.aov)


# Bonferroni correction
pairwise.t.test(SURVEY$Exercise, SURVEY$FavTime.fact, p.adj = "bonf")
```
