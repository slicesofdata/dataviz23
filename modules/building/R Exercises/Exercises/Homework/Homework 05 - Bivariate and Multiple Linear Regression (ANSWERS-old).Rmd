...
R^2^, for all of the reasons why the DV differs among participants in our sample, the predictors account for this much.

http://www.statisticssolutions.com/assumptions-of-multiple-linear-regression/    When the data is not normally distributed a non-linear transformation, e.g., log-transformation might fix this issue.
...
*ANSWER:*  b0 - y-intercept; the value of the criterion variable (y) when the predictor variable (x) is 0. b1 - the regression coefficient, or slope that represents the unit change (increase or decrease) in the criterion for each unit change in the predictor. 
...
*ANSWER:*  SSmodel = SStotal - SSresidul;  SSmodel/SStotal = .50
...
Regression model: Y-hat = b0 + b1*X
...
Ads and Airplay = *r* = .101; *p* = .15
Ads and Attract = *r* = .081; *p* = .26
...
*ANSWER:*  *R*-squared = .6647. This amount of variance accounted for my predictors is certainly better than not having predictors. Adjusted for bias in the sample, the poplulation adjusted R is slightly lower, but still .6595. Almost two-thrids of the variability in Sales is accounted for by the preditors together.  
...
9.10. **QUESTION:**  Use the appropriate function to test for multicolinearity. Use the suggested values to determine if you have multicolinearity.
...
We could also compare the models statistically using an ANOVA test. This shows that the model with 3 preditors is statisically better than the simple model.
