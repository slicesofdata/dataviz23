...
1. Describe the purpose of using bivariate linear regression. Include in your answer the type of data needed for linear regression analyses.
...
2. Identify the two coefficient values that are needed for a regression analysis and describe what they are.
...
3. The regression model is compared to another model, a simpler one. Identify what that model is and describe what the error in that model represents.  

**ANSWER:**



4. If the SStotal is 2000 and the SSresidual are 1000. What is the proportion of variance in the criterion is accounted for by the predictor? 
...
For a simple-linear regression there is:
  ...
  - The standardized regression coefficient equals the zero-order correlation.
...
```{r}
#hide ALBUM <- read.csv("Album Sales.csv") # Read in csv file and assign as data frame object
str(ALBUM) 
```
...
str(ALBUM2)
write.csv(ALBUM2, file = "Album Sales 2.csv")
...
5.3. Interpreting Specific Model Parameters (coefficents part of the table)
...
#library(moments)
#moments::agostino.test(ALBUM$Ads)
```
...
Using the plot() function on the model object that you created will produce model plots for examining the assumptions of the model. You can view each plot separately or produce a 2x2 chart that displays them together. However, grouping the plots will reduce their size for visual inspection. 
...
```{r}
str(ALBUM) # get the variables names

ALBUM.model2 <- lm(formula = Sales ~ Ads + Airplay + Attract, 
                   data = ALBUM, na.action = na.exclude)
```
