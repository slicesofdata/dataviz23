...
```{r}
#hide 
with(HEALTH, psych::describeBy(height, All))
```
...
1.7. The `cdc` data set contains data from very many people (20K). Let's still just assume that's a sample for this exercise. However, your samples will be much smaller of course. In order to calculate the standard error of the mean for the *z*-test, you will need to know the size of your sample. Use `length()` to obtain the number of people in your height variable and assign that to a variable named `Samplesize`.

**ANSWER:**

```{r}
#hide 
Samplesize <- with(HEALTH, length(height))

#take a look at it
Samplesize
```

1.8. Now use all of those pieces of information to plug into `one.sample.z()` from the `asbio` library below where there is a ??. For greatest flexibility, replace the ?? with the objects you created rather than the actual values. The data argument in the function stands for the sample variable you are testing, so I've started it for you by identifying the `HEALTH` data frame. You can copy your modified code and then put it in the `R` code block to execute. Remember all `R` code needs to be in those code blocks. 

Formula: `z test = (Sample mean - population mean) / (sigma / sqrt(n))`

Ex: `z.test(data = health$??, null.mu = ??, sigma = ??)`

**ANSWER:**
```{r}
#hide 
with(HEALTH, z.test(height, null.mu = Popmean, sigma = Popsd))
with(HEALTH, z.test(height, null.mu = 67.12, sigma = Popsd))
zzz
```
...
I would reject H0 because the *p*-value is less than an alpha of .05. The difference between the means observed from the data is unlikely due to random error for a sample of 20K. 
...
*ANSWER:*
```{r}
shapiro.test(Salmonella) # or lilliefors test

lattice::histogram(~Salmonella)
qqnorm(Salmonella); qqline(Salmonella)
```
...
*ANSWER:* The distribution does not deviate from that of a normal distribution, *S-W* = .98, *p* > .05; the assumption is met.  The salmonella at the factory is numerically higher (.46 MPN/g) than .3 MPN/g, but is not statistically different from that value, *t*(8) = 2.2, *p* > .05. The confidence interval also shows that the true mean for the factory may be below .3 MPN/g. The factory, however, should watch those levels because they are potentially high.
