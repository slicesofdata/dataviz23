...
## 1.0 T-Test for a Single Sample
...
The *t-test* for a single sample works basically the same way as the Z test. The 1-sample t-test does compare the mean of a single sample.  Unlike the other tests, the independent and dependent sample t-test it works with only one mean 
score.
...
Bring in Survey data set and conduct an independent samples t-test examining Males vs Females on how many hours a day they spend on social media. 

Q20 - How many hours a day do you spend on social media. 
...
```{r}
#hide
Independent <- read.csv("Independent.csv")
...
#hide
par(mfrow=c(2,2))
hist(Independent$Q20[Independent$Gender==0])
hist(Independent$Q20[Independent$Gender==1])
qqnorm(Independent$Q20[Independent$Gender==0])
qqnorm(Independent$Q20[Independent$Gender==1])
par(mfrow=c(1,1))
```
...
#hide
shapiro.test(Independent$Q20)
shapiro.test(Independent$Q20[Independent$Gender==0]) 
shapiro.test(Independent$Q20[Independent$Gender==1]) 
```
...
```{r}
#hide
t.test(Independent$Q20[Independent$Gender==0], Independent$Q20[Independent$Gender==1], alternative ="two.sided", paired = F, conf=.95, var.equal=F)
...
```{r}
#hide
wilcox.test(Independent$Q20[Independent$Gender==0], Independent$Q20[Independent$Gender==1], alternative ="two.sided", 
            paired = FALSE, conf.int = TRUE, conf.level = 0.95)
```
...
A clinic provides a program to help their clients lose weight and asks a consumer agency to investigate the effectiveness of the program. The agency takes a sample of 15 people, weighing each person in the sample before the program begins and 3 months later. Determine whether the program is effective.
