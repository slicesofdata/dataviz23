---
title: "Homework 03 - z-scores-dataframes-and-graphs"
author: "Answer Key"
...
**ANSWER:** Simply consider how many standard deviations each score is from their respective means. Welder = 63; mean of welders = 60, sd = 6. The welder is half of a standard deviation above the mean. Machinist = 59; mean of all machinists = 55, sd = 4. He is also above the mean by 1 standard deviation. Thus, the machinist seems to have shown more progress than the welder, relative of course, to other machinists and welders.
...
# 3. Above a z of .45
pnorm(.45, mean = 0, sd = 1, lower.tail = FALSE)
...
If we had z scores and we knew that our distribution was a standard normal distribution defined by a mean = 0 and sd = 1, we could also find that probability of someone scoring less than or equal to a z score of that `z70` object's value. For example: 
...
If this machine design is implemented, we can ask:
...
6.2. **QUESTION:** Use the appropriate `R` function to answer these two questions and assign that to the objects `f` and `m`. I have started this one for you by creating the objects, but you now need to use `pnorm()` and add the appropriate arguments to the function. Simply replace the 0 with your function. 
...
**ANSWER:**
```{r}
with(HEALTH, moments::kurtosis(age))   # or moments::kurtosis(HEALTH$age)
```

9.5. You can also use both `qqnorm()` and `qqline()` to plot a quantile-quantile plot (this is more precise than quartiles) so that you can examine whether the age variable is normal in shape; `qqline()` fits a line to the qqnorm plot. If the points fall along that line, age will be roughly normal in shape. Simply type the functions and put your variable object inside the parentheses as you did above. 
...
**ANSWER:**  No, this distribution appears to be skewed positively.
...
**ANSWER:** 
```{r}
with(HEALTH, levels(genhlth))   # or levels(HEALTH$genhlth)
```
...
**ANSWER:** 
```{r}
with(HEALTH, nlevels(genhlth))    # or nlevels(HEALTH$genhlth)
```
...
**ANSWER:**  Percent of total scores, not frequencies.
...
**ANSWER:** Counts (or frequency counts)
