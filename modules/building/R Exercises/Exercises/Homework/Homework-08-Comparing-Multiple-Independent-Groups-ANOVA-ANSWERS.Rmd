...
1.3. Using some real data, we can read in some data based on startup businesses. Based on startup costs for different businesses, we will analyze different DVs by comparing levels of different IVs. 

Read in "Jobs.csv" data file from Sakai and assign the contents to a data frame called `JOBS`:

```{r}
setwd("C:/Users/lklap/Dropbox/Exercises") #set working directory if necessary
...
Knowing the sample size of each of the groups is important for many reasons. For instance, you wouldn't want to run an *ANOVA* when you have really small sample sizes. You can count the samples sizes, but it's much easier to create a frequency table of the variable using `plyr::count()` from the `plyr` library so that you know the sample size for each level of a variable. 

- Ex: `with(dataframename, plyr::count(variablename)) )`
...
The first column ("x") indicates the type of `Shop` and the second column is frequency count for each type/level of `Shop`.
The levels of `Shop` are either 1 = Bakery, 2 = Pizzaria, or 3 = Shoes Store.


2.2. Now that we know the levels and sample sizes, we can create the factors using `factor()`. If we add two arguments to the function (e.g., levels and labels) we can replace the numbers with names that correspond to the levels of the IV. We will add ".fact" to the new variable so that we know these are our new factors. Let's practice with a simple three-level example. Remember that in order to add the variable to your existing data frame, you have to specify the data frame too. You can also create your data file with the level names too rather than with numbers, but you might not always obtain data files that are in the format you need them to be in, so let's practice. 
...
JOBS$Shop.fact <- with(JOBS, factor(Shop, 
                       levels = c(1, 2, 3), 
                       labels = c("Bakery", "Pizzaria", "Shoe Store")))
```
...
##3. Examine the Assumptions for the one-way ANOVA for idependent groups (between-subjects)
...
- Ex: `with(dataframename, psych::describeBY(DV, IV))`
...
- Ex: `with(dataframename, lattice::densityplot(~ DV | IV))` 
...
Besides looking at the data visually, the `shapiro.test()` will test for normality. However, the function does not have a built-in way to test normality for the subgroups or levels of a factor. However, `by()` will serve as a helper function to allow you to conduct the statistical test at each level of your factor.

- Ex: `with(dataframename, by(DV, IV, shapiro.test))`

```{r}
with(JOBS, by(Startup, Shop.fact, shapiro.test))

# to show you more of how the by() function works to repeat tasks, do the same using mean as the last argument
...
- Ex: `with(dataframename, bartlett.test(DV ~ IV))`
...
- `with(dataframename, car::leveneTest(DV, IV))`

```{r}
with(JOBS, car::leveneTest(Startup, Shop.fact))
```

If you notice from the title of the output, the default version of the Levene test is based on "medians" which is supposedly more robust and accurate than that based on the "mean". If you wanted to use means for the variance measure, you can simply use the *center* argument and set it equal to "mean". If the mean version makes more sense to you, use that one, but remember to use "center = mean" because by default "center = median". Understanding the nuances between these versions is not necessary at this point.

```{r}
with(JOBS, car::leveneTest(Startup, Shop.fact, center = mean))
```
...
- Ex: *F*(2, 38) = .7532, *p* = .895.



##4. Conducting the ANOVA test/examining model fit
...
-  *F* = between-groups variance (model)  /  within-groups variance (residual) 
...
4.2. In order to obtain the *F* ratio value, SS, degrees of freedom, and the *p*-value, the `aov()` can be used for fitting *ANOVA* models for a categorical IV; `aov()` follows the same format as `lm()`for regression. 

- Ex: `with(dataframename, aov(DV ~ IV))`
...
- Ex: `with(data frame, oneway.test(DV ~ IV))`
...
-  *Model/Between-groups df* (#groups - 1): 3 - 1 = 2

-  *Residual/Within-groups df* (n - 1): 
    1. Bakery: 13 - 1
    2. Pizzaria: 11 - 1
    3. Shoe Store: 10 - 1
    -  Total: 12 + 10 + 9 = 31

**Mean Squares of the model and the resdiuls**
...
As you can see this compares each of the groups to each other and shows you the differences as well as the *p*-value for each of the pairwise tests. In this case, there are no significant differences between startup costs for different type of shops because all of the *p*-values (p adj) are < .05 (alpha). 
...
Notice that the *p*-values are > .05 (alpha). In other words, both post-hoce approaches tell you the same story.



##7. Effect Size for ANOVA 

7.1. This post will look at effect size with *ANOVA*, which is not the same as other tests (like a *t*-test). When using effect size with *ANOVA*, we use a measure of *r*-squared, which reflects the ratio of variability that the model explains out of all the variability that exists in the dependent variable. In regression terminology, this was the coefficient of determination. This effect-size measure is sometimes also referred to as *Eta squared* in the context of ANOVA. 
...
*OPTIONAL:*
7.2. Because ANOVA is a special case of regression for which the predictor is categorical rather than continuous, we can still create a linear model using `lm()`. And, because you know the `R` output for a linear model provides you with r-squared measures, you can see also verify the that ANOVA and regession are the same. 

Build a linear model and examine the estimates:
```{r}
reg <- with(JOBS, lm(Startup ~ Shop.fact))
summary(reg)
```

Notice, that the (multiple) R-squared value in the output is the same as that calculated above.
...
If you want to create a graph of the conditions, one way to do so is to create a simple box-and-whisker plot for the levels of the factor by using the lattice package. Plot the DV as a function of the IV. For box-and-whisker plots, use `lattice::bwplot()`. They are nice because they convey a lot of information. A good summary and illustration is [here](http://flowingdata.com/2008/02/15/how-to-read-and-use-a-box-and-whisker-plot/)
...
2. **QUESTION:** Use `plyr::count()` on the Site variable to determine your sample size and examine of the structure of your dataframe. 
...
3. **QUESTION:** You may have noticed that `Site` is an integer variable. Let's change that because it refers to different site locations in New Mexico. Using `factor()` make the `Site` variable into a `factor` with labels that reflect: 1 = Uxmal, 2 = Cholula, and 3 = Coba.
...
4. After you have done that use `psych::describeBy()` to grab descriptive statistics for each group.
...
5. **QUESTION:** Now that we have the groups separated into factors it is time to start looking at the depth variable. Using `lattice::densityplot` and `shapiro.test` check to see if depth is normally distributed for the different groups. (Hint: `by()` will help you conduct the test acorss the group levels of the IV)

*CODED ANSWER:*
```{r}
with(DIG, lattice::densityplot(~ Depth | Site.fact))

with(DIG, by(Depth, Site.fact, shapiro.test))
```

6. Is the HOV assumption met?

*CODED ANSWER:*
```{r}
with(DIG, car::leveneTest(Depth, Site.fact))
```

*ANSWER:* Yes, the Levene's test shows that site locations likely have the same variances, *F*(2, 34) = .79, *p* = .46.


7. **QUESTION:** Now create the ANOVA model using `aov()` and name the model something so that you can compare the groups and then examine the model output to determine whether your ANOVA test reveals differences between groups.
...
8. **QUESTION:** Remember, the F value represents the ratio of the MS of the model to the MS of the residuals.  This ratio helps you determing whether the model using the IV to predict your data is better than not using a predictor. The *p*-value provides a likelihood measure of the likelihood the the data under the null hypothesis (e.g., the predictor does not help/groups have the same means). Do the locations have statistically different depths?

*ANSWER:* 

Yes, the sites statistically differ in depth, *F*(2, 34) = 19.91, *p* < .05. 



9. **QUESTION:** The result of the ANOVA determines whether to run a post-hoc test. For practice, run a post-hoc test to determine which factors levels would be significantly different and the direction of that difference. For example, use the Tukey HSD test to make all pairwise comparisons. 
...
Well, if I was looking at raw values and did not care about statistical significance, I would tell her to dig at either Cholula or Coba because the depths at Uxmal are deeper on average than both locations. Because the digs are based on samples, I could also tell her that based on the *p*-values for the Tukey's test, the Uxmal location has stastically deeper digs than do the other two locations and the depths at Cholula and Coba do not differ more than is expected due to chance. 
