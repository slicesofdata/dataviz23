---
title: "HW 6: t-tests and effect sizes"
...
*ANSWER:* The one-sample *t*-test is used to test whether a sample mean is equivalent to (or different from) a known population mean when the population standard deviation is not known. The dependent variable must be interval or ratio in scale and the distribution should be approximately normal in shape.



2. **QUESTION:** Describe the purpose of a paired-samples *t*-test.

*ANSWER:* The paired-samples *t*-test for is used to test whether the mean difference score for variable pairs is equivalent to (or different from) 0. The dependent variables must be interval or ratio in scale and the distribution of differences should be approximately normal in shape.
...
1.2.  Examining Data and Assumptions of *t*-tests. 
...
1.3. You can also use different statistical tests to examine whether the shape of distribution differs from the shape of a normal distribution. Some tests are included in the nortest library. We will use the Shapiro-Wilk test using shapiro.test() and the Lilliefors (Kolmogorov-Smirnov) test for normality using the lillie.test() function.


```{r}
# Load the nortest library if not loaded 
#library(nortest) 
...
1.4. If your data adhere to a fairly normal distribution, you can compare your sample mean to a population mean using the *t*-test. In order to calculate a one-sample *t*-test you must obtain the mean and standard deviation of the sample. You could do this manually as you have before, but functions will also accomplish this. 
...
1.5. However, there is an easier way to calculate the *t*-value and obtain the corresponding *p*-value. The t.test() function from the built-in stats library can be used to conduct the one-sample *t*-test. Of course, you will need to specify variable and the population mean arguments in the t.test() function.   
...
##3. Estimating the variance and standard deviation of the population##
...
# biased sd (square root of biased variance)
BP.sd.biased <- sqrt(BP.var.biased)
BP.sd.biased

# unbiased sd  (square root of unbiased variance)
...
The general formula is:  
  ...
  t.test(dependent variable 1, dependent variable 2, paired = TRUE)
...
Similar to the paired-samples *t*-test, the function takes the form:
  ...
  wilcox.test(dependent variable 1, dependent variable 2, paired = TRUE) 
...
```{r}
# define the function
cohens.d.two.sample <- function(xvar1, xvar2, paired=TRUE){
  if (!paired) { #if paired is FALSE
    pooledvar = 
    d = abs((mean(xvar1) - mean(xvar2)) / pooledvar) 
  } else {
    d = abs((mean(xvar1) - mean(xvar2)) / sd(xvar1 - xvar2)) 
  }
   return(d) # return the value of d
}
cohens.d.one.sample <- function(xvar, mu, sigma=TRUE){
  if (!sigma) { #if sigma is FALSE
   d = abs((mean(xvar) - mu) / sd(xvar)) # calculate using sd
  } else { #otherwise, sigma is TRUE
   d = abs((mean(xvar) - mu) / sigma) # calculate using sigma
  }
  return(d) # return the value of d
}
...
When you want to use your newly-created cohens.d.paired function, you can enter the arguments as needed:

```{r}
HAPPY.d <- cohens.d.two.sample(HAPPY$Pre, HAPPY$Post)
HAPPY.d
# enter the arguments and assigne the value returned from the calculation to an object named BLOOD.d
BLOOD.d <- cohens.d.one.sample(BLOOD$BP, 120, sigma = F)
...
```{r}
HAPPY.d <- abs((mean(HAPPY$Pre) - mean(HAPPY$Post)) / sd(HAPPY$Pre - HAPPY$Post))
HAPPY.d
...
SURVEY$ExercisePerDay <- ExercisePerWeek/7 # divide by 7 for days
...
*ANSWER:*
```{r}
tipping.t <- t.test(SURVEY$Tip, mu = 16.08026)
tipping.t
```
...
histogram(~SURVEY$Tip)
```


5.1.3. **QUESTION:** What do you conclude based on your test output? Be clear in your answer.

*ANSWER:* The distribution deviates from that of a normal distribution, *S-W* = .84, *p* < .05; the assumption is not met. Based on this result, the *t*-test result may not be stable. Based on the distribution, there may be some outlier to removes, but removing them will likely not change the shape of the distribution because most people tip about 15% and there is not much variability in the data. The *t*-test is likely not an appropriate test. 
...
*ANSWER:*
```{r}
t.test(SURVEY$ExercisePerDay, SURVEY$SocialMediaPerDay, paired = TRUE)

mean(SURVEY$ExercisePerDay); mean(SURVEY$SocialMediaPerDay)
```


5.2.2. **QUESTION:** What do you conclude based on your test output? Be clear in your answer.  

*ANSWER:* On average, people spend significantly fewer hours exercising per day (*M* = 2.36) than they do on social media (*M* = 1.11), *t*(74) = 5.39, *p* < .05.
