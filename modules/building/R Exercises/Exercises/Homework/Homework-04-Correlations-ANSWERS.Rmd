...
**ANSWER:**
```{r}
with(ICECREAM, lattice::histogram(~IceCreamSold, 
          main = "Histogram of Temperature", 
          xlab = "Temperature in Celcius"))
```
...
**ANSWER:**
```{r}
with(ICECREAM, lattice::xyplot(IceCreamSold ~ Temp,
...
**ANSWER:** The correlation appears to be positive. As temperature increases, ice cream sales increase.
...
# Mean
with(BOOK, mean(Books)); with(BOOK, mean(Attend)); with(BOOK, mean(Grade))
                                                         
# Median
with(BOOK, median(Books)); with(BOOK, median(Attend)); with(BOOK, median(Grade))

# Skewness
with(BOOK, moments::skewness(Books))
with(BOOK, moments::skewness(Attend))
with(BOOK, moments::skewness(Grade))

# kurtosis
with(BOOK, moments::kurtosis(Books))
with(BOOK, moments::kurtosis(Attend)) 
with(BOOK, moments::kurtosis(Grade))
```

Not required: You can also obtain a lot of this information using `psych::describe()` or `psych::describeBy()` if you want your data separated by groups. 
```{r}
with(BOOK, psych::describe(Books))
with(BOOK, psych::describe(Attend))
with(BOOK, psych::describe(Grade))

# look at descriprtive stats for Grade but for each level of Books (there are 4 levels).
with(BOOK, psych::describeBy(Grade, Books))
```
...
**ANSWER:**
```{r}
with(BOOK, xyplot(Grade ~ Attend, 
       main = "Grades as a Function of Attendance", 
       xlab = "Attendance", 
       ylab = "Grades"))

# if you want to add the line of best fit
with(BOOK, xyplot(Grade ~ Attend, type = c("p","r"),
       main = "Grades as a Function of Attendance", 
       xlab = "Attendance", 
       ylab = "Grades"))
```
...
**ANSWER:**
```{r}
with(BOOK, cor(Grade, Attend))

with(BOOK, cor.test(Grade, Attend))
```

The Pearson correlation between Grade and Attendance is .48.


6.5. What is your alpha level and the *p*-value for the correlation? Is the correlation statistically significant?
**ANSWER:**

My alpha level is .05. The *p*-value for the correlation is .002. Because the probability that the sample of 40 people (df = 38) will be correlated with *r* = .48. The probability of obtaining a correlation of at least .48 for the sample if the populations data were not correlated is .002. Therefore, attendance is significantly correlated with grades because *p*-value is less than alpha. 
...
**WRITTEN ANSWER:**
The coefficient of determination is .23, which means that class attendance accounts for 23% of the variability in grades. 
