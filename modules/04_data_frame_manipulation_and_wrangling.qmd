...
Make note of any warnings that appear when loading libraries. There are some libraries that contain have function with the same names. Be aware that the most recently loaded library function will take precedence. You can avoid confusion using `::` to call a function from a particular library (e.g., `libraryname::functionname()`).
...
You can reference variables by their name or their column position in a data frame (e.g., 1, 25, etc.). 
...
Why separate lines if you can use `&`? Well, separate function calls written as separate lines of code also make code inclusion/exclusion easy.
