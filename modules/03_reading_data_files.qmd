...
We will use a URL of a zip file to download to the project data directory, unzip the contents, and remove the `.zip` file. 
...
DAT <- openxlsx::read.xlsx(URL)
```

What does the head of the file look like?

```{r}
head(DAT)
```

Just an FYI, when you want a different worksheet, `openxlsx::read.xlsx()` in **{openxlsx}** `r packageVersion("openxlsx")` won't work. In many cases, however, you will have the data file saved locally. In this case, we saved it as part of the download process.
...
If you pass that previous path string (`r file` ), you will overwrite that existing Excel file, which likely is not your end goal for at least two reasons. First, the `.csv` would be incorrectly named as `.xlsx` and second, you would completely clobber your Excel spreadsheet along with all of the other worksheets that you did not read. Yeah, that would be bad.
...
When working with teams, you would likely also manage a github repo for the project. Team contributors would be working on separate tasks in separate files. The files might be used by all team members. You could imagine how difficult the process would be if there was only one file with all project code. 
