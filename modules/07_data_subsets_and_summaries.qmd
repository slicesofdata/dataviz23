...
OK, pass the string to read `sheet = "swim"` from the file.

```{r}
DAT <- openxlsx::read.xlsx(paste(base_url, file_name, sep = ""), sheet = "swim")
```
...
ATHENA_NAME_count
```
We can see that the top counts of all time are by `r ATHENA_NAME_count[1, "name"]` for a total of `r `ATHENA_NAME_count[1, "count"]`. Nice work `r stringr::str_split_fixed(ATHENA_NAME_count[1, "name"], " ", 2)[1]`!
...
STAG_NAME_count
```
We can see that the top counts of all time are by `r STAG_NAME_count[1, "name"]` for a total of `r `STAG_NAME_count[1, "count"]`. Go  `r stringr::str_split_fixed(STAG_NAME_count[1, "name"], " ", 2)[1]`!


To wrap up this example, sometimes working with separate data frames as a result of `filter()` can provide more useful summaries.
