...
1. Find the path to the Git program executable that was installed in an earlier step.
     ...
     * If for some reason you don't see a path listed using that approach, type:  `Sys.which("git")` in your R console. The path here will likely be truncated so you will have to fill in the gaps when performing the next step.
