...
Tell RStudio where to find the Git program.

1. In RStudio, go to **Tools > Global Options** and click on Git/SVN
2. Check the option to *Enable version control interface for RStudio projects* if not selected.
3. Find the path to the Git program executable that was installed in an earlier step.
     * In the *Terminal* in RStudio (not the R console), you could type `where git` on Windows or `which git` on Mac/Linux and you might find the path easily.
       - For Windows Users, this is likely in `C:/Program Files (x86)/Git/bin/git.exe` or in `C:/Program Files/Git/bin/git.exe` 
       - For Linux/OS X Users, the path should be something like: `/usr/bin/git`

4. Set the path to the Git program executable if not there. Click *Apply* and *OK*.
...
3. When you create the project, a directory will be created, a name will auto populate (e.g., 'dataviz23-stewart'). If you change the name, name it something that you will know as your team project. In order to keep the class organized, I might suggest you create the project in your PSYC167 course directory. You should already have a R project for you homework called something like `homework.Rproj` in that course directory.
...
6. **Changing, Committing, and Pushing Again**
...
1. Git Clients: [GitKraken](https://www.gitkraken.com/)
