...
For this homework, you will save the `R Markdown` (e.g., `.Rmd`) homework document and save it to your `homework` project directory on your computer. Save a new copy of this file so that its name has the prefix `2023_Lastname_FirstInitial`. Full name example: `2023_cookg_EX_04.Rmd`. Feel free to work with a partner but understand that the work submitted has to be your own.
...
The homework utilizes the modules below. 
...
Make now of where the data files were unpacked.
...
After saving and unpacking the data, use **{readr}** along with **{here}** to read the `2010_CMS_Athena_Outdoor_100.csv` file and assign it to an object of some name. You might wish to examine whether column names exist or do not and make any necessary function argument modifications.
...
After viewing, comment out your above code block so that the output will not appear in your knitted output file. Alternatively, select the gear icon from in your code block and change the output option to "show nothing (run code)" or "show nothing (do not run code)".  



# *Getting Data Frame Column Names* 

Use the function discussed earlier to obtain the names of the columns.
...
Find the *length* of the elements in the column names:
...
There are many ways to select data from a data frame. You don't know how you might want to filter, so get some practice by reviewing some options as presented in the module on data frame manipulation. 

Take your data frame and subset it so that your function returns columns according to the following approaches. 


## Two columns by their exact name:
...
Take your data frame and filter so that your function returns rows according to the following approaches. 
...
## Filter by times under 14 seconds during the month of April and sort the data frame so that the fastest times are at the top: 
...
## Filter by times that are faster that the mean of all times: 

```{r}

```

## Create a new variable that represents the mean of all times, then filter to include only times that are faster than that mean, and only for meets in April.
...
Summarize the data so that you have the mean across all runners:
...
Summarize the data so that you have the mean of time for each of the meet locations and then arrange the data frame by the fastest meet locations:
