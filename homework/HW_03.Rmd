...
First, we need to get all the TFRRS data. There is an `.R` script file to perform the download of a `"tfrrs.zip"`, so download that script and save it. Make a note of where this code is and then source `"download_tfrrs_data.R"` either inside the code block below or in the `R` console. 
...
Make note of where `"download_tfrrs_data.R"` was saved and where the data were unpacked. 
...
```{r}
DAT <- readr::read_csv(here::here("data", "tfrrs", "2010_CMS_Athena_Outdoor_100.csv"))
```
